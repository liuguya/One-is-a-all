...
class ReadQuestViewController: BaseViewController {
    ...
    //文章model
    private var detalModle:QuestDetailModel?
    
    //相关推荐model
    private var recommendModel:QuestRecommendModel?
    //评论model
    ...
    func createPinglunDownload(){
        if titleId != nil{
            ...
            let urlString = String(format: "\(questionCommentUrl)%@/%@", titleId!,page!)
    ...
    func createTuijianDownload(){
        if titleId != nil {
            let urlString = String(format: "\(questionRecommendUrl)%@", titleId!)
            let downlod = Download()
            downlod.delegate = self
            downlod.type = 20
            downlod.getWithUrl(urlString)
    ...
    func createDownload(){
        if titleId != nil{
            //ProgressHUD.showOnView(self.view)
            let urlString = String(format: "\(questionUrl)%@", titleId!)
            let downlod = Download()
            downlod.delegate = self
            downlod.type = 10
            downlod.getWithUrl(urlString)
...
//MARK:Download代理
extension ReadQuestViewController:DownloadDelegate{
    ...
    func downloader(downloader: Download, didFinishWithData data: NSData?) {
        ...
        if downloader.type == 10{
            ...
            if let jsonData = data{
                let model = QuestDetailModel.parseModel(jsonData)
                //print(model)
                //回到住线程刷新UI
            ...
            if let jsonData = data{
                ...
                let model = QuestRecommendModel.parseModel(jsonData)
                //print(model)
                dispatch_async(dispatch_get_main_queue(), {
                    //回到主线程
...
//MARK:uitableView
extension ReadQuestViewController:UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0{
            //文章信息
            if indexPath.row == 0{
                if detalModle?.data != nil{
                    cell = QuestDetailCell.createQuestDetailCellFor(tableView, atIndexPath: indexPath, withModel: detalModle!)
                }
                
            }
        }else if indexPath.section == 1{
            if recommendModel != nil{
                cell = ReadDetailRecommendCell.createQuestRecommendFor(tableView, atIndexPath: indexPath, withModel: recommendModel!)
            }
    ...
    func tableView(tableView: UITableView, heightForRowAtIndexPath indexPath: NSIndexPath) -> CGFloat {
        ...
        if indexPath.section == 0{
            ...
            if indexPath.row == 0{
                if detalModle?.data != nil{
                    height = QuestDetailCell.heightWithComment(detalModle!.data!)
                }
            }
        }else if indexPath.section == 1{
            return 100
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if indexPath.section == 1{
            if recommendModel?.data?.count > 0{
                ...
                let readDetailCtrl = ReadQuestViewController()
                readDetailCtrl.titleId = model?.question_id!
                readDetailCtrl.titleName = "问答"
